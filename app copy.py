        ...
        st.markdown(
            ...
            <div style="background-color:{bg_color}; padding:10px; border-radius:10px; height:500px; display:flex; flex-direction:column; justify-content:space-between;">
                ...
                <div style="margin-top:10px;">
                    ...
                    <p style="color:gray; font-size:14px; margin:0;">:round_pushpin: {location}</p>
...
if top_recommendation:
    ...
    for i, recommendation in enumerate(top_recommendation):
        if recommendation:  # Check if the recommendation is valid
            # Extract relevant data for display
            place_name = recommendation.get('name', 'Unknown')
            location = recommendation.get('formatted_address', 'Unknown location')
            rating = recommendation.get('rating', 'N/A')
            review_count = recommendation.get('user_ratings_total', '0')
            place_url = recommendation.get('url', 'https://maps.google.com')  # Extracting the URL

            # Get the image URL
            photo_reference = recommendation.get('photos', [{}])[0].get('photo_reference', '')
            image_url = (
